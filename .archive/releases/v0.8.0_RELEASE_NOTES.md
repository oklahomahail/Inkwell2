...
- **Smart Deduplication**: Identical content isn't saved twice—reduces network noise and improves performance
- **Unmount Flush**: Unsaved changes are persisted when you navigate away, preventing data loss
- **Visual Feedback**: Real-time status indicator shows "Saving…" → "Saved" → or error state

**User Experience Impact**:

- Seamless background saves—no interruptions while writing
...
When the same chapter is edited in multiple tabs or after offline→online transitions:
...
- **`useAutoSave`** — Trailing-only debounce with unmount flushing
...
- **`connectivityService`** — Monitors online/offline + queues writes
- **`snapshotService`** — Versioned backup management
- **`enhancedStorageService`** — Intelligent persistence layer
- **`autosaveService`** — Coordinated editor-wide autosave state

### UI Components

- **`AutosaveIndicator`** — Status badge in topbar
- **`BackupRestoreDialog`** — Browse and restore snapshots
- **`ConflictDialog`** — Choose conflict resolution strategy
...
- ✅ Editor no longer loses changes mid-edit
- ✅ Offline mode works smoothly; queued saves sync automatically
- ✅ Duplicate save prevention reduces server load
