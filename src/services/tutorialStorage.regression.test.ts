...
describe('TutorialStorage Regression Tests', () => {
  ...
  beforeEach(() => {
    ...
    // Set up mock profile
    mockedUseProfile.mockReturnValue(createMockUseProfileReturn(createTestProfile()));
  ...
  describe('No Active Profile Scenarios', () => {
    ...
    it('should handle database unavailable gracefully', async () => {
      ...
      const failingDb = {
        ...
        // New interface methods
        getItem: vi.fn().mockRejectedValue(new Error('Database unavailable')),
        setItem: vi.fn().mockRejectedValue(new Error('Database unavailable')),
        removeItem: vi.fn().mockRejectedValue(new Error('Database unavailable')),
        getAllKeys: vi.fn().mockRejectedValue(new Error('Database unavailable')),
      };
      mockedUseMaybeDB.mockReturnValue(failingDb as any);
      mockedUseProfile.mockReturnValue(
        createMockUseProfileReturn(createTestProfile('test-profile')),
      );
  ...
  describe('Profile Switch Scenarios', () => {
    it('should maintain profile isolation during switches', async () => {
      ...
      // Switch to profile 2
      mockedUseProfile.mockReturnValue(
        createMockUseProfileReturn(createTestProfile('profile-2', 'Profile 2')),
      );
  ...
  describe('Storage Failure Scenarios', () => {
    it('should handle database write failures gracefully', async () => {
      ...
      const failingDb = {
        ...
        // New interface methods
        getItem: vi.fn().mockRejectedValue(new Error('Database write failed')),
        setItem: vi.fn().mockRejectedValue(new Error('Database write failed')),
        removeItem: vi.fn().mockRejectedValue(new Error('Database write failed')),
        getAllKeys: vi.fn().mockRejectedValue(new Error('Database write failed')),
      };
