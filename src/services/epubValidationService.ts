...
class EPUBValidationService {
  ...
  private validateContent(project: EnhancedProject, chapters: Chapter[], issues: EPUBValidationIssue[]) {
  ...
  private validateStructure(chapters: Chapter[], issues: EPUBValidationIssue[]) {
    ...
    if (chapters.length > 1) {
      ...
      if (wordCounts.length > 1) {
        ...
        const hasExtremeLengths = wordCounts.some(count => 
          count < avg * 0.2 || count > avg * 5
        );
  ...
  private validateManuscriptFormatting(project: EnhancedProject, chapters: Chapter[], issues: EPUBValidationIssue[]) {
    ...
    if (totalWordCount < 50000) {
      issues.push({
        ...
        severity: 'minor',
        suggestion: 'Most novels are 70,000-100,000 words. Verify this aligns with your target genre.'
      });
      ...
      issues.push({
        ...
        severity: 'minor',
        suggestion: 'Consider if the manuscript could be tightened. Very long first novels are harder to sell.'
      });
    }
  ...
  private validateManuscriptContent(project: EnhancedProject, chapters: Chapter[], issues: EPUBValidationIssue[]) {
  ...
  private determineReadinessLevel(score: number, issues: EPUBValidationIssue[]): 'not-ready' | 'needs-work' | 'good' | 'excellent' {
  ...
  private hasPlaceholderText(project: EnhancedProject): boolean {
    ...
    return (project.chapters || []).some(chapter =>
      chapter.scenes?.some(scene => placeholderPattern.test(scene.content || ''))
    );
  }
}
