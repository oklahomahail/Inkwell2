...
Your entire response must be valid JSON only. Do not include any text outside the JSON structure.`;
  ...
  async generateOutline(premise: StoryPremise): Promise<GeneratedOutline> {
    try {
      console.log('🧪 USING MOCK VERSION - Generating outline for:', premise.title);
      ...
      console.log('✅ Generated mock outline successfully:', mockOutline);
      return mockOutline;
    } catch (error) {
      console.error('❌ Mock generation failed:', error);
      throw new Error('Failed to generate story outline. Please try again.');
    }
  }
