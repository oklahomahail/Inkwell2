// src/services/characterConsistencyAnalyzer.ts - Character trait and behavior consistency checking
...
class CharacterConsistencyAnalyzer {
  ...
  async analyzeCharacterConsistency(
    ...
    try {
      ...
      // Generate AI prompt for character consistency analysis
      ...
      const issues = await this.parseCharacterAnalysisResponse(response.content, text, project);
  ...
  private extractCharacterReferences(text: string, project: EnhancedProject): string[] {
    ...
    // Find character names in the text
    project.characters.forEach((character) => {
      ...
      // Check for common nicknames or aliases
      if (character.backstory || character.description) {
  ...
  private buildCharacterContext(project: EnhancedProject, characterIds: string[]): string {
    const relevantCharacters = project.characters.filter((char) => characterIds.includes(char.id));

    const context = ['CHARACTER PROFILES:'];
    ...
    relevantCharacters.forEach((char) => {
      ...
      if (char.goals) {
        context.push(`Goals/Motivations: ${char.goals}`);
      }

      if (char.conflicts) {
        context.push(`Internal/External Conflicts: ${char.conflicts}`);
      }

      if (char.backstory) {
        context.push(`Backstory: ${char.backstory}`);
      }
...
Focus on characters: ${characterIds.map((id) => this.getCharacterNameById(characterIds, id)).join(', ')}
...
Return an empty array [] if no consistency issues are found. Be thorough but focus on significant inconsistencies that would break character believability.`;
    ...
    try {
      ...
      parsedIssues.forEach((issue, index) => {
        ...
        const character = project.characters.find(
          (char) => char.name.toLowerCase() === issue.characterName.toLowerCase(),
        );

        if (!character) return;

        // Find the position of the text segment
        const segmentIndex = plainText.toLowerCase().indexOf(issue.textSegment.toLowerCase());

        if (segmentIndex === -1) return;

        issues.push({
          id: `character-${character.id}-${index}-${Date.now()}`,
          characterId: character.id,
          characterName: character.name,
          type: issue.type || 'personality-contradiction',
          ...
          endPos: segmentIndex + issue.textSegment.length,
          evidence: {
  ...
  private getCharacterNameById(characterIds: string[], characterId: string): string {
    // This would need to be implemented based on your character storage
