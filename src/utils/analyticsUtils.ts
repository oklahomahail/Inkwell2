...
export function analyzeTimeOfDayPatterns(
  ...
  const hourlyData = new Array(24)
    .fill(null)
    .map((_, hour) => ({ hour, productivity: 0, count: 0 }));
...
export function analyzeDayOfWeekPatterns(
  sessions: WritingSession[],
): { day: string; productivity: number }[] {
 ...
 */
export function analyzeSessionLengthPatterns(
  sessions: WritingSession[],
 ...
 */
export function analyzeWordCountDistribution(
  sessions: WritingSession[],
  ...
  const message =
    daysNeeded === 1
      ...
      : daysNeeded <= 7
        ? `In ${daysNeeded} days`
        : daysNeeded <= 30
          ? `In ${Math.ceil(daysNeeded / 7)} weeks`
...
export function calculateConsistency(sessions: WritingSession[]): number {
  ...
  const totalDays = Math.max(
    1,
    Math.ceil((Date.now() - new Date(firstSession.date).getTime()) / (1000 * 60 * 60 * 24)),
  );
