...
class SearchBenchmarkHarness {
  ...
  async runBenchmark(config: BenchmarkConfig): Promise<BenchmarkResult> {
    ...
    // Mock storage service for Node.js environment
    ...
    // Create mock implementations that don't use localStorage
    const mockStorageService = {
      loadProject: () => this.corpus!.project,
      loadWritingChapters: async (_projectId: string) => this.corpus!.chapters,
    };

    try {
      // Override storage methods
      (storageService as any).loadProject = mockStorageService.loadProject;
      (storageService as any).loadWritingChapters = mockStorageService.loadWritingChapters;
