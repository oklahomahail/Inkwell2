...
const ClaudeProvider: React.FC<ClaudeProviderProps> = ({ children }) => {
  ...
  const sendMessage = useCallback(async (
    ...
    try {
      ...
      if (context) {
        const contextInfo = [];
        if (context.currentProject) contextInfo.push(`Current project: ${context.currentProject}`);
  ...
  const suggestContinuation = useCallback(async (currentText: string): Promise<string> => {
...
Please provide 2-3 sentences that would logically follow this text, maintaining consistency in voice, tense, and narrative style.`;
  ...
  const improveText = useCallback(async (text: string, instructions?: string): Promise<string> => {
...
Provide an improved version that enhances clarity, flow, and impact while maintaining the original intent and voice.`;
  ...
  const analyzeCharacter = useCallback(async (characterName: string, context?: string): Promise<string> => {
...
Be specific and actionable for a fiction writer.`;
  ...
  const generatePlotIdeas = useCallback(async (currentPlot?: string): Promise<string> => {
