...
import professionalExportService, { EXPORT_TEMPLATES } from '@/services/professionalExportService';
...
const ExportQualityDashboard: React.FC<ExportQualityDashboardProps> = ({ className = '' }) => {
  ...
  const getReadinessColor = (level: string) => {
    switch (level) {
      ...
      case 'needs-work': return 'text-yellow-600 bg-yellow-50 dark:bg-yellow-900/20';
      case 'not-ready': return 'text-red-600 bg-red-50 dark:bg-red-900/20';
  ...
  const getSeverityIcon = (severity: string) => {
    switch (severity) {
      ...
      case 'minor': return <Info className="w-4 h-4 text-yellow-500" />;
      default: return <Info className="w-4 h-4 text-gray-500" />;
    }
  };

  const handleExport = async (format: 'docx' | 'epub', template: string = 'standard-manuscript') => {
    if (!currentProject) return;
    ...
    try {
      ...
      if (format === 'docx') {
        const result = await professionalExportService.exportDOCX(currentProject, chapters, template);
        if (result.success) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Manuscript Readiness</h3>
          <div className={`px-3 py-1 rounded-full text-sm font-medium ${getReadinessColor(report.readinessLevel)}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="text-center">
            <div className="text-3xl font-bold text-green-600">{report.formatting.pageEstimate}</div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Pre-Export Checklist</h3>
            ...
            <div key={index} className="flex items-start gap-3 p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50">
              <div className="flex-shrink-0 mt-0.5">
                {item.completed ? (
                  ...
                  <XCircle className={`w-5 h-5 ${item.required ? 'text-red-500' : 'text-yellow-500'}`} />
                )}
              </div>
              
              <div className="flex-1">
                <div className={`font-medium ${item.completed ? 'text-green-700 dark:text-green-400' : 'text-gray-900 dark:text-white'}`}>
                  {item.item}
                  {item.required && !item.completed && (
                    <span className="ml-2 text-xs px-2 py-0.5 bg-red-100 text-red-700 rounded">REQUIRED</span>
                  )}
                </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div className="flex items-center gap-2">
            <CheckCircle className={`w-4 h-4 ${report.metadata.hasTitle ? 'text-green-500' : 'text-red-500'}`} />
            <span>Title</span>
          </div>
          
          <div className="flex items-center gap-2">
            <CheckCircle className={`w-4 h-4 ${report.metadata.hasTableOfContents ? 'text-green-500' : 'text-red-500'}`} />
