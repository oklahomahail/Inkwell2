// src/components/Claude/ClaudeAssistant.tsx
...
const ClaudeAssistant: React.FC<ClaudeAssistantProps> = ({ selectedText }) => {
  ...
  const { currentProject, activeView } = useWritingPlatform();
  ...
  const handleQuickAction = async (action: string) => {
    ...
    try {
      ...
      // Create a manual assistant message for quick actions
      ...
      // We'll need to expose this through the context
      // For now, send it as a regular message
      await sendMessage(`${action === 'continue' ? 'Continue this text' : 'Improve this text'}: "${selectedText}"`, context);
    } catch (error) {
  ...
  return (
    <div className={`fixed bottom-6 right-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl z-50 transition-all duration-300 ${
