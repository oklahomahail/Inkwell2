// src/components/Planning/RelationshipMapper.tsx - Interactive relationship network visualization
...
export default function RelationshipMapper({
  ...
  // Generate nodes and edges from character data
  const { nodes, edges } = useMemo(() => {
    ...
    characters.forEach((character, index) => {
      ...
      else if (character.role === 'supporting') nodeColor = '#F59E0B'; // amber
      ...
      if (layoutMode === 'circular') {
        ...
        const levelMap = { protagonist: 0, antagonist: 0, supporting: 1, minor: 2 };
        const level = levelMap[character.role as keyof typeof levelMap] || 2;
        x = (index % 3) * (canvasWidth / 3) + canvasWidth / 6;
        y = level * (canvasHeight / 4) + canvasHeight / 8;
      } else {
        // Force-directed layout (simple random positioning)
        x = Math.random() * (canvasWidth - 100) + 50;
        y = Math.random() * (canvasHeight - 100) + 50;
      }

      const connectionCount = character.relationships?.length || 0;

      nodeMap.set(character.name, {
        id: character.name,
        name: character.name,
        role: character.role,
        x,
    ...
    characters.forEach((character) => {
      character.relationships?.forEach((rel, relIndex) => {
        if (nodeMap.has(rel.withCharacter)) {
          let edgeColor = '#6B7280'; // gray
          if (rel.type === 'ally')
          ...
          else if (rel.type === 'mentor') edgeColor = '#8B5CF6'; // purple

          const strength =
            rel.arcInfluence === 'major' ? 3 : rel.arcInfluence === 'moderate' ? 2 : 1;

          edgeList.push({
            id: `${character.name}-${rel.withCharacter}-${relIndex}`,
            source: character.name,
            target: rel.withCharacter,
            type: rel.type,
            strength,
            color: edgeColor,
            arcInfluence: rel.arcInfluence,
          });
        }
      });
    });
  ...
  const filteredEdges = useMemo(() => {
    if (relationshipFilter === 'all') return edges;
    return edges.filter((edge) => edge.type === relationshipFilter);
  ...
  const drawCanvas = (ctx: CanvasRenderingContext2D) => {
    ...
    filteredEdges.forEach((edge) => {
      ...
      if (sourceNode && targetNode) {
        ...
        ctx.globalAlpha = selectedEdge === edge.id ? 1 : 0.7;
        ...
        if (showLabels) {
          ...
          ctx.fillText(edge.type.replace('_', ' '), midX, midY - 5);
        }
      }
    });
    ...
    nodes.forEach((node) => {
      ctx.beginPath();
      const radius = Math.max(15, node.connections * 3);
      ...
      if (showLabels) {
        ...
        ctx.fillText(node.role, node.x, node.y + radius + 28);
      }
    });
  ...
  useEffect(() => {
    ...
    drawCanvas(ctx);
  }, [nodes, filteredEdges, zoom, panX, panY, selectedNode, selectedEdge, showLabels]);

  const relationshipTypes = [
    { id: 'all', name: 'All Relationships', color: '#6B7280' },
    { id: 'ally', name: 'Allies', color: '#10B981' },
    { id: 'enemy', name: 'Enemies', color: '#EF4444' },
    { id: 'love_interest', name: 'Love Interests', color: '#EC4899' },
    { id: 'mentor', name: 'Mentors', color: '#8B5CF6' },
    { id: 'family', name: 'Family', color: '#F59E0B' },
  ];

  const selectedCharacter = selectedNode
    ? characters.find((char) => char.name === selectedNode)
    : null;
    ...
    >
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Interactive visualization of character relationships and connections
            </p>
          </div>
      ...
      <div className="flex">
        ...
        <div className="flex-1 p-4">
          <div
            ref={containerRef}
            className="relative border border-gray-200 dark:border-gray-600 rounded"
          >
            <canvas
              ref={canvasRef}
              width={800}
        ...
        {/* Sidebar */}
        <div className="w-80 border-l border-gray-200 dark:border-gray-600">
          {/* Legend */}
          <div className="p-4 border-b border-gray-200 dark:border-gray-600">
                  ...
                  <div>
                    ...
                    <div className="space-y-1 max-h-40 overflow-y-auto">
                      {selectedCharacter.relationships.map((rel, index) => (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <span className="font-medium">{rel.withCharacter}</span>
                            <span
                              className="px-2 py-1 rounded-full"
                              style={{
                                backgroundColor:
                                  relationshipTypes.find((t) => t.id === rel.type)?.color + '20',
                                color: relationshipTypes.find((t) => t.id === rel.type)?.color,
                              }}
                            >
                              {rel.type.replace('_', ' ')}
                            </span>
          ...
          {/* Network Stats */}
          <div className="p-4">
            <h3 className="font-medium text-gray-900 dark:text-white mb-3">Network Stats</h3>
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                <span className="text-gray-600 dark:text-gray-400">Most Connected:</span>
                <span className="font-medium text-gray-900 dark:text-white text-xs">
                  {nodes.sort((a, b) => b.connections - a.connections)[0]?.name || 'None'}
                </span>
              </div>
            </div>
          </div>
