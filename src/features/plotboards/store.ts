...
export const usePlotBoardStore = create<PlotBoardStore>()(
  subscribeWithSelector(
    (set, get) => ({
      ...
      importBoard: async (boardData: Partial<PlotBoard>) => {
        ...
        if (boardData.settings)
          board.settings = { ...DEFAULT_BOARD_SETTINGS, ...boardData.settings };
      ...
      getCardsByStatus: (boardId: string, status: PlotCardStatus) => {
        ...
        return board.columns.flatMap((column) =>
          column.cards.filter((card) => card.status === status),
        );
      },
        ...
        try {
          ...
          for (const update of syncResult.updates) {
            await get().updateCard(update.cardId, {
              ...
              notes: update.sceneUpdates.notes,
              wordCount: update.sceneUpdates.wordCount,
              tags: update.sceneUpdates.tags || [],
            });
          }
          ...
          const errorMessage =
            error instanceof Error ? error.message : 'Failed to sync with chapters';
          set({ lastError: errorMessage });
          throw error;
        }
      },
      ...
      createCardsFromChapters: async (
        ...
        try {
          ...
          let targetColumn = board.columns.find((col) => col.type === 'chapter');
          if (!targetColumn && board.columns.length > 0) {
            targetColumn = board.columns[0];
          }

          if (!targetColumn) {
            targetColumn = await get().createColumn(boardId, 'Scenes', 'chapter');
          }
      ...
      autoSyncWithTimeline: async (
        ...
        try {
          ...
          const errorMessage =
            error instanceof Error ? error.message : 'Failed to sync with timeline';
          set({ lastError: errorMessage });
          throw error;
        }
      },
    ...
    {
      name: 'plot-boards-store',
      storage: createJSONStorage(() => storage),
      partialize: (state) => ({
        boards: state.boards,
        activeBoard: state.activeBoard,
        autoSyncConfig: state.autoSyncConfig,
        templates: Object.fromEntries(
          Object.entries(state.templates).filter(([, template]) => !template.isBuiltIn),
        ),
      }),
    },
  ),
