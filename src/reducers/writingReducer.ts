// Step 3: Update your existing writing reducer
// src/reducers/writingReducer.ts

import { Scene, Chapter, SceneStatus } from '../types/writing';
...
export type WritingAction =
  ...
  | { type: 'UPDATE_SCENE_CONTENT'; payload: { sceneId: string; content: string; wordCount: number } }
  ...
  switch (action.type) {
    ...
    case 'UPDATE_SCENE_CONTENT':
      return {
        ...
        currentScene: state.currentScene?.id === action.payload.sceneId
          ? {
              ...state.currentScene,
              content: action.payload.content,
              wordCount: action.payload.wordCount,
